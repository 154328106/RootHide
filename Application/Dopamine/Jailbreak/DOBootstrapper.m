...
- (void)prepareBootstrapWithCompletion:(void (^)(NSError *))completion
{
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:basebinPath]) {
        if (![[NSFileManager defaultManager] removeItemAtPath:basebinPath error:&error]) {
            // Dopamine 3.0 - 3.0.6 could leave dyld.old undeletable after a
            // jbupdate panic. Move it out of the active preboot tree, then retry.
            BOOL recovered = NO;
            ...
            if ([[NSFileManager defaultManager] fileExistsAtPath:corruptedFilePath] &&
                ...
                NSString *orphanedName = [NSString stringWithFormat:@"orphaned-%06X", arc4random_uniform(0x1000000)];
